...
def load_experiment(fname="effData.txt", get_eff_data=False):
    colnames = [
        'toolName', 'targetName', 'actionId', 'initialObjPos[0]',
        'initialObjPos[1]', 'initialObjImgPos.x', 'initialObjImgPos.y',
        'finalObjectPos[0]', 'finalObjectPos[1]', 'finalObjImgPos.x',
        'finalObjImgPos.y'
    ]
    ...
    return (mvec, vvec, weight, mdist,
            np.vstack([diffx, diffy]).T) if get_eff_data else (mvec, vvec,
                                                               weight, mdist)
...
def call_simulator():
    ...
    physicsClient = p.connect(
        PYBULLET_INSTANCE)  # or p.DIRECT for non-graphical version
...
def cost(x, y):
    return (100 * (np.linalg.norm(x.mean(0) - y.mean(0)) +
                   np.linalg.norm(x.var(0) - y.var(0))))**2
...
def optimize(param_names, fname, object_name="yball", opt_f=gp_minimize):
    dbounds = {
        'lateralFriction': (0.01, 5.0),
        'spinningFriction': (1.0e-4, 5.0),
...
if __name__ == "__main__":
    ...
    call_simulator()
    param_names = ['lateralFriction', 'rollingFriction',
                   'mass']  #, 'xnoise', 'ynoise']
